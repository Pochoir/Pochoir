#ifndef SYMBOLIC_WALK_HPP
#define SYMBOLIC_WALK_HPP
...
inline void heterogeneity<N_RANK>::symbolic_space_cut_interior(int t0, int t1, 
...
template <int N_RANK> template <typename F>
inline void heterogeneity<N_RANK>::symbolic_space_cut_boundary(int t0, int t1, 
				grid_info<N_RANK> const & grid, zoid_type * parent, F const & f)
{
    queue_info *l_father;
    queue_info circular_queue_[2][ALGOR_QUEUE_SIZE];
...
template <int N_RANK> template <typename F>
inline void heterogeneity<N_RANK>::symbolic_space_time_cut_interior(int t0, 
...
template <int N_RANK> template <typename F>
inline void heterogeneity<N_RANK>::symbolic_space_time_cut_boundary(int t0, 
		int t1,	grid_info<N_RANK> const & grid, zoid_type * parent, 
		int child_index, F const & f)
{
    const int lt = t1 - t0;
...
// #pragma cilk_grainsize = 1
                cilk_for (int j = 0; j < queue_len_[curr_dep_pointer]; ++j) {
                    ...
                    // assert all the sub-grid has done N_RANK spatial cuts 
                    assert(l_son->level == -1);
					assert(child_index < projection_zoid->children.size()) ;
					zoid_type * child = projection_zoid->children [child_index];
...
template <int N_RANK> template <typename F>
inline void heterogeneity<N_RANK>::heterogeneous_space_cut_interior(int t0, 
				int t1, grid_info<N_RANK> const & grid, 
				zoid_type * projection_zoid, F const & f)
{
    queue_info *l_father;
...
inline void heterogeneity<N_RANK>::heterogeneous_space_cut_boundary(int t0, 
			int t1,	grid_info<N_RANK> const & grid, 
...
template <int N_RANK> template <typename F>
inline void heterogeneity<N_RANK>::heterogeneous_space_time_cut_interior(int t0,
				int t1, grid_info<N_RANK> const & grid, 
				zoid_type * projection_zoid, F const & f)
{
    const int lt = t1 - t0;
...
template <int N_RANK> template <typename F, typename BF>
inline void heterogeneity<N_RANK>::heterogeneous_space_time_cut_boundary(int t0,
					int t1,	grid_info<N_RANK> const & grid, 
					zoid_type * projection_zoid, F const & f, BF const & bf)
{
    const int lt = t1 - t0;
