 ...
 * ============================================================================
 */
#ifndef AUTO_TUNING_WITH_SPAN_SAWZOID_HPP 
#define AUTO_TUNING_WITH_SPAN_SAWZOID_HPP
...
inline void auto_tune<N_RANK>::symbolic_sawzoid_space_cut_interior_span(int t0,
int t1, grid_info<N_RANK> const & grid, unsigned long parent_index, 
F const & f, int * num_zoids, double & redundant_time, 
double & projected_time_span, double & elapsed_time_span, 
double & max_loop_time, int bits)
{
    ...
    for (int curr_dep = 0; curr_dep < N_RANK+1; ++curr_dep) {
		...
		double elapsed_time_dep = 0 , ptime_dep = 0 ;
...
// #pragma cilk_grainsize = 1
				...
				double clock_time = tdiff2(&end, &start) ;
				redundant_time += clock_time ;
				double elapsed_time = clock_time - rtime ;
				if (elapsed_time_dep + ptime_dep < elapsed_time + ptime)
				{
					elapsed_time_dep = elapsed_time ;
					ptime_dep = ptime ;
				}
		...
		elapsed_time_span += elapsed_time_dep ;
		projected_time_span += ptime_dep ;
#if !USE_CILK_FOR
//        cilk_sync;
#endif
...
inline void auto_tune<N_RANK>::symbolic_sawzoid_space_cut_boundary_span(int t0,
		int t1, grid_info<N_RANK> const & grid, unsigned long parent_index, 
		...
		double & max_loop_time)
{
	...
	if (sim_can_cut) 
	{
...
#endif
		{
			...
			clock_gettime(CLOCK_MONOTONIC, &start1);
			/* cut into space */
			double time = 0, rtime = 0, ptime_span = 0, 
					elapsed_time_span = 0, elapsed_time = 0  ;
			symbolic_sawzoid_space_cut_interior_span(t0, t1, grid, index, f, 
							num_subzoids, rtime, ptime_span,
							elapsed_time_span, time, i) ;
			clock_gettime(CLOCK_MONOTONIC, &end1);
			max_loop_time = max(time, max_loop_time) ;
			elapsed_time = tdiff2(&end1, &start1) - rtime + 
							elapsed_time_span ;
			//span of children = elapsed_time_span + ptime_span
			assert (elapsed_time >= 0.) ;
			assert (ptime_span >= 0.) ;
			assert (rtime >= 0.) ;
			if (elapsed_time + ptime_span < space_cut_elapsed_time + 
											space_cut_ptime)
			{
				space_cut_elapsed_time = elapsed_time ;
				space_cut_ptime = ptime_span ;
				best_case = i ;
...
double & max_loop_time)
{
    ...
    if (sim_can_cut) 
	{
...
#endif
		{
			...
			if (call_boundary) 
			{
				symbolic_sawzoid_space_cut_boundary_span(t0, t1, l_father_grid, 
					index, f, bf, num_subzoids, rtime, ptime_span,
					elapsed_time_span, time, i) ;
			}
			else
			{
				symbolic_sawzoid_space_cut_interior_span(t0, t1, l_father_grid, 
					index, f, num_subzoids, rtime, ptime_span, 
					elapsed_time_span, time, i) ;
			}
			clock_gettime(CLOCK_MONOTONIC, &end1);
			max_loop_time = max(time, max_loop_time) ;
