#ifndef AUTO_TUNING_TRAP_HPP
#define AUTO_TUNING_TRAP_HPP

#include "auto_tuning_header2.hpp"
...
inline void auto_tune<N_RANK>::symbolic_space_cut_interior(int t0, int t1, 
...
template <int N_RANK> template <typename F>
inline void auto_tune<N_RANK>::symbolic_space_time_cut_interior(int t0, 
		...
		int child_index, F const & f)
{
	...
	//unsigned long key = (unsigned long) bottom_volume << NUM_BITS_IN_INT 
	//									| top_volume;
	//unsigned long key = (unsigned long) bottom_volume << 42 
	//									| top_volume << 21 | lt ;
...
//#ifndef NDEBUG
#if 0
inline void auto_tune<N_RANK>::homogeneous_space_cut_interior(int t0, 
...
//#ifndef NDEBUG
#if 0
inline void auto_tune<N_RANK>::homogeneous_space_cut_boundary(int t0, 
				int t1, grid_info<N_RANK> const & grid, 
				zoid_type * projection_zoid, F const & f)
#else
inline void auto_tune<N_RANK>::homogeneous_space_cut_boundary(int t0, 
...
//#ifndef NDEBUG
#if 0
inline void auto_tune<N_RANK>::homogeneous_space_time_cut_interior(int t0, 
					int t1, grid_info<N_RANK> const & grid, 
					zoid_type * projection_zoid, F const & f)
#else
inline void auto_tune<N_RANK>::homogeneous_space_time_cut_interior(int t0, 
...
//#ifndef NDEBUG
#if 0
inline void auto_tune<N_RANK>::homogeneous_space_time_cut_boundary(int t0, 
					int t1,	grid_info<N_RANK> const & grid, 
					zoid_type * projection_zoid, F const & f)
#else
inline void auto_tune<N_RANK>::homogeneous_space_time_cut_boundary(int t0, 
...
template <int N_RANK> template <typename F>
inline void auto_tune<N_RANK>::heterogeneous_space_cut_interior(int t0, 
				int t1, grid_info<N_RANK> const & grid, 
				zoid_type * projection_zoid, F const & f)
{
    queue_info *l_father;
...
inline void auto_tune<N_RANK>::heterogeneous_space_cut_boundary(int t0, 
			int t1,	grid_info<N_RANK> const & grid, 
...
template <int N_RANK> template <typename F>
inline void auto_tune<N_RANK>::heterogeneous_space_time_cut_interior(int t0,
				int t1, grid_info<N_RANK> const & grid, 
				zoid_type * projection_zoid, F const & f)
{
    const int lt = t1 - t0;
...
template <int N_RANK> template <typename F, typename BF>
inline void auto_tune<N_RANK>::heterogeneous_space_time_cut_boundary(int t0,
					int t1,	grid_info<N_RANK> const & grid, 
					zoid_type * projection_zoid, F const & f, BF const & bf)
{
    const int lt = t1 - t0;
	...
	else if (lt > l_dt_stop)  //time cut
	{
        ...
        if (call_boundary) {
            heterogeneous_space_time_cut_boundary(t0, t0+halflt, l_son_grid, 
								&(m_zoids [index]), f, bf);
								//projection_zoid->children [0], f, bf);
        } else {
            heterogeneous_space_time_cut_interior(t0, t0+halflt, l_son_grid, 
								&(m_zoids [index]), f);
								//projection_zoid->children [0], f);
        ...
        if (call_boundary) {
            heterogeneous_space_time_cut_boundary(t0+halflt, t1, l_son_grid, 
								&(m_zoids [index]), f, bf);
								//projection_zoid->children [1], f, bf);
        } else {
            heterogeneous_space_time_cut_interior(t0+halflt, t1, l_son_grid, 
								&(m_zoids [index]), f);
								//projection_zoid->children [1], f);
