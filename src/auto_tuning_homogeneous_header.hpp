...
#endif
#ifdef CHECK_CACHE_ALIGNMENT
      if ((z->ref_point * m_type_size) % 64 == (ref_point * m_type_size) % 64)
#endif
      {
        index = start1->second ;
        return true ;
...
#endif
#ifdef CHECK_CACHE_ALIGNMENT
    if ((centroid * m_type_size) % 64 == (z->ref_point  * m_type_size) % 64)
#endif
    {
      index = start->second ;
      return true ;
...
void dfs(unsigned long node, vector <zoid_type> & temp_zoids,
  ...
  if (z.decision == (decision_type) 1 << (zoid_type::NUM_BITS_DECISION - 2) ||
    ...
    for (int i = 0 ; i < z.num_children ; i++) {
      ...
      if (color [z.children [i]] == ULONG_MAX) {
        //node is white
...
void print_dag()
{
  ...
  for (int j = 0 ; j < m_head.size() ; j++)
  {
    ...
    while (!que.empty())
    {
      ...
      else if (z->decision & m_space_cut_mask)
      {
        cout << " space " <<__builtin_ctz((int) z->decision & m_space_cut_mask) - 1 ; 
      }
      else if (z->decision & 1 << (zoid_type::NUM_BITS_DECISION - 2))
      {
        cout << " loop " ;
      }
      else
      ...
      cout << endl ;
      if (z->decision & 1 << (zoid_type::NUM_BITS_DECISION - 1))
      {
        cout << " boundary " << endl ;
      }
      else
      ...
      cout  << " predicted time " << z->time << endl ;
  ...
  m_algo.power_of_two_time_cut(t0, t0 + h1, grid, f, bf) ;
    ...
    while (h2 >= 1)
    {
      ...
      cout <<	" offset " << offset << " offset + h " <<
              offset + h << " h " << h << endl ;
      m_head.push_back (ULONG_MAX) ;
      build_auto_tune_dag_sawzoid(offset, offset + h, grid, f, bf, index) ;
          ...
          TF const & tf, void * array, int depth, void ** array_addr)
{
  ...
  clock_nanosleep(CLOCK_MONOTONIC, 0, &start, &end) ;
  cout << flush ;
  assert (t0 < t1) ;
  int T = t1 - t0 ;
  if (t0 >= t1)
