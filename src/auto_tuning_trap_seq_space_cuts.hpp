...
inline void auto_tune<N_RANK>::loop_interior(int t0, INT_TYPE t1, 
...
inline void auto_tune<N_RANK>::loop_boundary(int t0, INT_TYPE t1, 
	...
	time_type & child_time, F const & f, time_type & best_time)
{
...
#endif
  {
    ...
    for (int i = 0 ; i < 2 ; i++)
    {
      ...
      bound_loop_time = max(m_zoids [child_index].loop_time, bound_loop_time);
#endif
#ifdef SUBSUMPTION3
...
#ifdef SUBSUMPTION3
    bound_loop_time = max (bound_loop_time, sum_loop_time) ;
#endif
#ifdef MEASURE_COLD_MISS
  ...
  if (m_zoids [index].num_children == 2)
  {
    //time cut
    unsigned long child_index = m_zoids [index].children [0] ;
    int l1 = m_zoids [child_index].num_level_divide ;
    child_index = m_zoids [index].children [1] ;
    int l2 = m_zoids [child_index].num_level_divide ;
    m_zoids [index].num_level_divide = max(l1, l2) ;
  }
  else if (m_zoids [index].num_children == 3)
  {
    //space cut
    unsigned long child_index = m_zoids [index].children [0] ;
    int l1 = m_zoids [child_index].num_level_divide ;
    child_index = m_zoids [index].children [1] ;
    int l2 = m_zoids [child_index].num_level_divide ;
    child_index = m_zoids [index].children [2] ;
    int l3 = m_zoids [child_index].num_level_divide ;
    m_zoids [index].num_level_divide = max(l1, max(l2, l3)) ;
  }
  ...
  */
  ...
  if (divide_and_conquer && divide_and_conquer_time < bound_loop_time)
  {
    force_divide = true ;
  }
#endif
...
#if defined (SUBSUMPTION_TIME) || defined (SUBSUMPTION3)
  m_zoids [index].loop_time = bound_loop_time ;
#endif
	...
	time_type & child_time, F const & f, BF const & bf, time_type & best_time)
{
  ...
  time_type time_cut_elapsed_time = 0, space_cut_elapsed_time = LONG_MAX ;
  time_type bound_loop_time = 0 ;
  //divide in time if zoid is in the interior or if zoid cannot be 
  //divided in space.
  if (lt > l_dt_stop && (! call_boundary || ! space_cut))
  {
    divide_and_conquer = true ;
    time_cut = true ;
  ...
  */
  ...
  else 
  {
    ...
    //compare loop time with divide and conquer time
    //if zoid is in the interior
    //if (! call_boundary)
    {
      //determine the looping time on the zoid
      LOOP_BOUNDARY ;
...
#define SPACE_CUT_INTERIOR2(k_, x0_, x1_, dx0_, dx1_) \
{ \
	l_son_grid.x0[i] = x0_; \
	l_son_grid.dx0[i] = dx0_; \
	l_son_grid.x1[i] = x1_ ; \
	l_son_grid.dx1[i] = dx1_ ; \
	trap_space_time_cut_interior(t0, t1, l_son_grid, projection_zoid->children [k_], f); \
}
			...
			unsigned long zoid_index, F const & f)
{
  ...
  thres = (slope_[i] * lt);
    ...
    if (cut_lb) {
            ...
            //process the middle gray zoid
            SPACE_CUT_INTERIOR2(0, l_start + mid - thres, l_start + mid + thres, slope_[i], -slope_[i]) ;
            SPACE_CUT_INTERIOR2(1, l_start, l_start + mid - thres, grid.dx0[i], slope_[i]) ;
            SPACE_CUT_INTERIOR2(2, l_start + mid + thres, l_end, -slope_[i], grid.dx1[i]) ;
    } // end if (cut_lb) 
    else { // cut_tb 
            const int mid = tb/2;
            l_son_grid = grid;
            ...
            const int l_end = (grid.x1[i]);
            const int ul_start = (grid.x0[i] + grid.dx0[i] * lt);
            SPACE_CUT_INTERIOR2(0, l_start, ul_start + mid, grid.dx0[i], -slope_[i]) ;
            SPACE_CUT_INTERIOR2(1, ul_start + mid, l_end, slope_[i], grid.dx1[i]) ;
            SPACE_CUT_INTERIOR2(2, ul_start + mid, ul_start + mid, -slope_[i], slope_[i]) ;
    } // end if (cut_tb) 
  }
...
#define SPACE_CUT_BOUNDARY2(k_, x0_, x1_, dx0_, dx1_) \
l_son_grid.x0[i] = x0_; \
l_son_grid.dx0[i] = dx0_; \
l_son_grid.x1[i] = x1_ ; \
l_son_grid.dx1[i] = dx1_ ; \
if (call_boundary) \
{ \
	trap_space_time_cut_boundary(t0, t1, l_son_grid, projection_zoid->children [k_], f, bf); \
} \
else \
{ \
	trap_space_time_cut_interior(t0, t1, l_son_grid, projection_zoid->children [k_], f); \
		...
		F const & f, BF const & bf)
{
  ...
  for (int i = N_RANK-1; i >= 0; --i) 
  {
    bool l_touch_boundary = touch_boundary(i, lt, l_father_grid) ;
#ifndef NDEBUG
#if defined (HOMOGENEOUS) || defined (FULL_ST_INVARIANCE_BOUNDARY)
    dim_touching_bdry [i] = false ;
#else
  ...
  if (cut_lb) {
    ...
    const int l_end = l_father_grid.x1[i];
    //process the middle gray zoid
    SPACE_CUT_BOUNDARY2(0, l_start + mid - thres, l_start + mid + thres, slope_[i], -slope_[i]) ;
    SPACE_CUT_BOUNDARY2(1, l_start, l_start + mid - thres, l_father_grid.dx0[i], slope_[i]) ;
    SPACE_CUT_BOUNDARY2(2, l_start + mid + thres, l_end, -slope_[i], l_father_grid.dx1[i]) ;
  } // end if (cut_lb) 
  else { // cut_tb 
    if (lb == phys_length_[i] && l_father_grid.dx0[i] == 0 && l_father_grid.dx1[i] == 0) {
       /* initial cut on the dimension */
            ...
            const int l_start = l_father_grid.x0[i];
            const int l_end = l_father_grid.x1[i];
            SPACE_CUT_BOUNDARY2(0, l_start, l_end, slope_[i], -slope_[i]);
            SPACE_CUT_BOUNDARY2(1, l_end, l_end, -slope_[i], slope_[i]);
    } else { /* NOT the initial cut! */
            const int mid = tb/2;
            l_son_grid = l_father_grid;
            const int l_start = l_father_grid.x0[i];
            const int l_end = l_father_grid.x1[i];
            const int ul_start = l_father_grid.x0[i] + l_father_grid.dx0[i] * lt;
            SPACE_CUT_BOUNDARY2(0, l_start, ul_start + mid, l_father_grid.dx0[i], -slope_[i]) ;
            SPACE_CUT_BOUNDARY2(1, ul_start + mid, l_end, slope_[i], l_father_grid.dx1[i]) ;
            SPACE_CUT_BOUNDARY2(2, ul_start + mid, ul_start + mid, -slope_[i], slope_[i]) ;
    }
  } // end if (cut_tb) 
  }
  else if (projection_zoid->decision & 1)
