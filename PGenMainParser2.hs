...
pGetGuardTilesTerm l_mode l_order l_rank l_color l_reg_GTs =
    let l_guards = map fst l_reg_GTs
        ...
        l_null_tile = if null l_tiles' 
                         then emptyTile
                         else let l_t = pSetTileOp PNULL $ head l_tiles'
                                  l_t_kernels = tKernels l_t
                                  l_t_kernel = if null l_t_kernels
                                                  then emptyKernel
                                                  else head l_t_kernels
                                  l_t_kernel' = l_t_kernel { kName = "__Pochoir_NULL__",
                                                             kFunc = (kFunc l_t_kernel) {
                                                                 kfName = "__Pochoir_NULL_"},
                                                             kTileSizes = [1],
                                                             kIndex = [0] }
                              in  l_t { tName = "__Pochoir_NULL__", 
                                        tSizes = [1], 
                                        tKernels = [l_t_kernel'] }
                         -- else emptyTile { tSizes = [1] }
