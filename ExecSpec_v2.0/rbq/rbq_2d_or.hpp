/*
 * ============================================================================
 *		 Filename:  rbq_2d_or.hpp
 *    Description:  Range bit query implementation to find the "or"
 ...
 * ============================================================================
 */
#ifndef RBQ_2D_OR_HPP
#define RBQ_2D_OR_HPP

#include "rbq_2d.hpp"

template <class word_type>
class rbq_2d_or : public rbq_2d <word_type>
{
public :
	//constructor
	rbq_2d_or(word_type * row_input, unsigned long n1, unsigned long n2,
...
protected :
	...
	virtual void init(word_type * a) {*a = 0 ;}

} ;


template <class word_type>
void rbq_2d_or<word_type>::initialize_skew_data()
{
	...
	//initialize the skewed input structure to all zeros.
...
A bit in result is the or of the corresponding bits in the query range. 
*/
template <class word_type>
void rbq_2d_or<word_type>::query(word_type * p_C, unsigned long p_n1,
							...
							unsigned long p_N, int * result)
{
	...
	if (i > 0 && k > 0)
	{
		...
		{
			result [p] = op((word_type) result [p], 
					  (c2 [l] - c2 [k - 1] - c1 [l] + c1 [k - 1] > 0)) ;
			c1 += p_N ;
			c2 += p_N ;
		}
	}
	else if (i > 0)  //k = 0
	{
		...
		for (unsigned int p = 0 ; p < this->q ; p++)
		{
			result [p] = op(result [p], (c2 [l] - c1 [l] > 0)) ;
			c1 += p_N ;
			c2 += p_N ;
		}
	}
	else if (k >0) //i = 0
	{
		for (unsigned int p = 0 ; p < this->q ; p++)
		{
			result [p] = op(result [p], (c2 [l] - c2 [k - 1] > 0)) ;
			c2 += p_N ;
		}
	}
	else //i = 0 and k = 0
...
//Used to check the correctness of query results of a rectangular query.
template <class word_type>
void rbq_2d_or<word_type>::query_without_preprocessing(unsigned long i, 
							unsigned long j, unsigned long k, unsigned long l,
							int * result)
{
 ...
 */
template <class word_type>
inline int rbq_2d_or<word_type>::query_1d_without_preprocessing(
                    ...
                    word_type * p_input)
{
    ...
    if (c + 1 <= d - 1)
    {
        ...
        return answer ||
            get_subword(i, this->WORD_SIZE - 1, p_input [c], 
						this->WORD_SIZE) > 0 ||
            get_subword(0, j, p_input [d], this->WORD_SIZE) > 0 ;
    }
    else
    {
        if (c < d)
        {
            ...
            return get_subword(i, this->WORD_SIZE - 1, p_input [c], 
							 this->WORD_SIZE) > 0 ||
                get_subword(0, j, p_input [d], this->WORD_SIZE) > 0 ;
        }
        else
        {
            ...
            assert(c == d) ;
            return get_subword(i, j, p_input [c], this->WORD_SIZE) > 0 ;
        }
    }
}
