...
int main(int argc, char * argv[])
{
    ...
    Pochoir_Array_1D(stagger) a(M), b(M);
    ...
    Pochoir_Kernel_1D(animwave_fn, t, i)
        a(t, i).v = a(t-1, i).v + cdtdx * (a(t-1, i+1).u - a(t-1, i).u);
        a(t, i).u = a(t-1, i).u + cdtdx * (a(t-1, i).v - a(t-1, i-1).v);
    Pochoir_Kernel_End

    for (int times = 0; times < TIMES; ++times) {
        gettimeofday(&start, 0);
        leap_frog.Run(nt, animwave_fn);
        gettimeofday(&end, 0);
    ...
    for (int times = 0; times < TIMES; ++times) {
        ...
        for (int t = 1; t <= nt; ++t) {
            cilk_for (int i = 0; i < M; ++i) {
                ...
                b(t, i).u = b(t-1, i).u + cdtdx * (b(t-1, i).v - b(t-1, i-1).v);
            }
        }
        gettimeofday(&end, 0);
        min_tdiff = min(min_tdiff, (1.0e3 * tdiff(&end, &start)));
