...
template <int N_RANK> 
inline void Algorithm<N_RANK>::find_homogeneity(Grid_Info<N_RANK> const & grid)
{
	Homogeneity h = (*color_region_) (grid) ;
    (*homogeneity_vector_).push_back_unique(h);
}
...
// #pragma cilk_grainsize = 1
                cilk_for (int j = 0; j < queue_len_[curr_dep_pointer]; ++j) {
                    ...
                    bicut_interior(l_son->t0, l_son->t1, l_son->grid);
                } /* end cilk_for */
                queue_head_[curr_dep_pointer] = queue_tail_[curr_dep_pointer] = 0;
                queue_len_[curr_dep_pointer] = 0;
#else
                ...
                if (queue_len_[curr_dep_pointer] == 0)
                    ...
                    bicut_interior(l_father->t0, l_father->t1, l_father->grid);
                else
                    //cilk_spawn plan_bicut(l_father->t0, l_father->t1, l_father->grid, region_n);
                    cilk_spawn bicut_interior(l_father->t0, l_father->t1, 
											l_father->grid);
#endif
            } else {
                /* performing a space cut on dimension 'level' */
...
inline void Algorithm<N_RANK>::space_bicut_boundary(int t0, int t1, 
												Grid_Info<N_RANK> const grid)
{
    queue_info *l_father;
    queue_info circular_queue_[2][ALGOR_QUEUE_SIZE];
...
// #pragma cilk_grainsize = 1
                cilk_for (int j = 0; j < queue_len_[curr_dep_pointer]; ++j) {
                    ...
                    bicut_boundary(l_son->t0, l_son->t1, l_son->grid);
                } /* end cilk_for */
                queue_head_[curr_dep_pointer] = queue_tail_[curr_dep_pointer] = 0;
                queue_len_[curr_dep_pointer] = 0;
#else
                ...
                if (queue_len_[curr_dep_pointer] == 0) {
                    ...
                    bicut_boundary(l_father->t0, l_father->t1, l_father->grid);
                } else {
                    //cilk_spawn plan_bicut_p(l_father->t0, l_father->t1, l_father->grid, region_n);
                    cilk_spawn bicut_boundary(l_father->t0, l_father->t1, 
												l_father->grid);
                }
#endif
            } else {
...
// #pragma cilk_grainsize = 1
                ...
                if (queue_len_[curr_dep_pointer] == 0)
                    ...
                    cilk_spawn space_time_cut_interior(l_father->t0, 
												l_father->t1, l_father->grid);
#endif
            } else {
                /* performing a space cut on dimension 'level' */
                pop_queue(curr_dep_pointer);
...
// #pragma cilk_grainsize = 1
                ...
                if (queue_len_[curr_dep_pointer] == 0) {
                    ...
                    cilk_spawn space_time_cut_boundary(l_father->t0, 
												l_father->t1, l_father->grid);
                }
#endif
            } else {
...
/* This is the version for interior region cut! */
template <int N_RANK> 
inline void Algorithm<N_RANK>::bicut_interior(int t0, int t1, 
								Grid_Info<N_RANK> const grid)
{
    ...
    if (sim_can_cut) {
        ...
        space_bicut_interior(t0, t1, grid);
        return;
        ...
        bicut_interior(t0, t0+halflt, l_son_grid);
        ...
        bicut_interior(t0+halflt, t1, l_son_grid);
        return;
    } else {
        // base case
...
inline void Algorithm<N_RANK>::bicut_boundary(int t0, int t1, 
											Grid_Info<N_RANK> const grid)
{
	int region_n = num_kernels - 1 ; //use the white clone
    const int lt = t1 - t0;
    ...
    if (sim_can_cut) {
        if (call_boundary) 
            ...
            space_bicut_boundary(t0, t1, l_father_grid);
        else
            //plan_space_bicut(t0, t1, l_father_grid, region_n);
            space_bicut_interior(t0, t1, l_father_grid);
        return;
    } 
    ...
    if (lt > l_dt_stop && lt > l_unroll) {
        ...
        if (call_boundary) {
            ...
            bicut_boundary(t0, t0+halflt, l_son_grid);
        } else {
            //plan_bicut(t0, t0+halflt, l_son_grid, region_n);
            bicut_interior(t0, t0+halflt, l_son_grid);
        }
        ...
        if (call_boundary) {
            ...
            bicut_boundary(t0+halflt, t1, l_son_grid);
        } else {
            //plan_bicut(t0+halflt, t1, l_son_grid, region_n);
            bicut_interior(t0+halflt, t1, l_son_grid);
        }
											...
											Grid_Info<N_RANK> const grid)
{
    ...
    if (sim_can_cut) {
        ...
        space_time_cut_interior(t0+halflt, t1, l_son_grid);
        return;
    } else {
        // base case
												...
												Grid_Info<N_RANK> const grid)
{
    ...
    if (lt > l_dt_stop && lt > l_unroll) {
        ...
        if (call_boundary) {
            ...
            space_time_cut_boundary(t0+halflt, t1, l_son_grid);
        } else {
            //plan_cut(t0+halflt, t1, l_son_grid, region_n);
            space_time_cut_interior(t0+halflt, t1, l_son_grid);
        }
...
#endif
            (*opks_[region_n]).cond_bkernel_[0](t0, t1, l_father_grid);
			//Pochoir_Base_Kernel<N_RANK> * cond_bkernel_ = opks_[region_n]->cond_bkernel_ ;
			//(*cond_bkernel_)(t0, t1, l_father_grid);
        } else {
            assert((t1 - time_shift_) % l_unroll == 0 && (t0 - time_shift_) % l_unroll == 0); 
#ifdef DEBUG_CUT_T_ALIGNMENT 
            printf("<plan_cut_p> bkernel_[%d] l_unroll = %d, lt(%d, %d)\n", 
